 ...
 * @since date
 */
public class ReadinessOutput extends AbstractSensorOutput<Sensor> implements Runnable{

    private static final String SENSOR_OUTPUT_NAME = "Readiness";
    private static final String SENSOR_OUTPUT_LABEL = "Readiness";
    private static final String SENSOR_OUTPUT_DESCRIPTION = "Daily Readiness Data";

    private static final Logger logger = LoggerFactory.getLogger(ReadinessOutput.class);
     ...
     */
    ReadinessOutput(Sensor parentSensor) {
    ...
    void doInit(String token, Date start, Date end) {
        ...
        dataStruct = sweFactory.createRecord()
                ...
                .addField("readiness", sweFactory.createQuantity()
                        .definition(SWEHelper.getCfUri("readiness"))
                        .label("Readiness"))
                .addField("recovery", sweFactory.createQuantity()
                        .definition(SWEHelper.getCfUri("recovery"))
                        .label("Recovery"))
                .build();
...
//            String endDate = config.timeFilter.endTime.toInstant().atZone(zoneOffset).toLocalDate().toString();
            ...
            String requestString = "https://api.ouraring.com/v2/usercollection/daily_readiness?start_date=";
            ...
            String readiness_response = makeRequest(requestString, bearerToken);
            JSONObject[] readiness_jsons = getDataRecord(readiness_response);

            int i =0;
            while (i < readiness_jsons.length) {
                ...
                if(!readiness_jsons[i].isNull("score") && !readiness_jsons[i].getJSONObject("contributors").isNull("recovery_index")) {
                    LocalDateTime date = LocalDateTime.parse(readiness_jsons[i].getString("day") + "T07:00:00");
                    dataBlock.setDoubleValue(0, date.toEpochSecond(zoneOffset));
                    dataBlock.setIntValue(1, readiness_jsons[i].getInt("score"));
                    dataBlock.setIntValue(2, readiness_jsons[i].getJSONObject("contributors").getInt("recovery_index"));
                    latestRecord = dataBlock;
                    latestRecordTime = date.toEpochSecond(zoneOffset);
                    eventHandler.publish(new DataEvent(latestRecordTime, ReadinessOutput.this, dataBlock));
                }
